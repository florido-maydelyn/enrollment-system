...
switch ($action) {
  ...
  case "list":
    $res = $conn->query("
      SELECT term_id, term_code, start_date, end_date
      FROM tblterm
      WHERE is_deleted = 0
  ...
  // ===================== ADD =====================
  case "add":
    $code = $_POST['term_code'];
    $start = $_POST['start_date'];
    $end = $_POST['end_date'];

    try {
      $stmt = $conn->prepare("
        INSERT INTO tblterm (term_code, start_date, end_date, is_deleted)
        VALUES (?, ?, ?, 0)
      ");
      $stmt->bind_param("sss", $code, $start, $end);
      $stmt->execute();

      $new_id = $conn->insert_id;
      $stmt2 = $conn->prepare("SELECT * FROM tblterm WHERE term_id = ?");
      $stmt2->bind_param("i", $new_id);
      $stmt2->execute();
      $result = $stmt2->get_result();
      $new_term = $result->fetch_assoc();

      echo json_encode(["success" => true, "term" => $new_term]);
    } catch (mysqli_sql_exception $e) {
      if (str_contains($e->getMessage(), "Duplicate entry")) {
        echo json_encode(["success" => false, "error" => "Term code already exists."]);
      } else {
  ...
  case "edit":
    try {
      $stmt = $conn->prepare("
        UPDATE tblterm 
        SET term_code = ?, start_date = ?, end_date = ?
